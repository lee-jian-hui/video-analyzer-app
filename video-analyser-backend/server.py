...
class VideoAnalyzerService(video_analyzer_pb2_grpc.VideoAnalyzerServiceServicer):
        ...
        try:
            ...
            try:
                ...
                if getattr(_Cfg, 'ENABLE_PRE_SUMMARIZE_ON_OVERFLOW', True):
                    ...
                    if changed:
                        logger.info("🧠 Applied pre-summarization/truncation to fit context budget")
                        full_message = fitted
            except Exception as e:
    ...
    def ResumeSession(self, request, context):
        ...
        logger.info(f"⏮️  ResumeSession called for video_id: {video_id}")
        ...
        try:
            ...
            logger.info(f"✅ Resumed session {video_id} → {video_path}")
            return video_analyzer_pb2.ResumeResponse(
                success=True,
            ...
            logger.error(f"❌ ResumeSession error: {e}", exc_info=True)
            context.set_details(str(e))
            context.set_code(grpc.StatusCode.INTERNAL)
            return video_analyzer_pb2.ResumeResponse(
                success=False,
